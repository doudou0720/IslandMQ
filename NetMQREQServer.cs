...
namespace IslandMQ
{
    ...
    public class NetMQREQServer(string endpoint = "tcp://127.0.0.1:5555") : IDisposable
    {
        ...
        private static string CreateSuccessResponse(string message, object? data = null, long requestId = 0, int statusCode = 200)
        {
            var response = new
            {
                success = true,
        ...
        private static string CreateErrorResponse(string errorMessage, long requestId = 0, int statusCode = 500)
        {
            var response = new
            {
                success = false,
        ...
        {
            ResponseSocket? socket = Interlocked.Exchange(ref _serverSocket, null);
            if (socket != null)
            {
                try
                {
                ...
                {
                    _logger?.LogError(ex, "Error closing socket: {Message}", ex.Message);
                }
                try
                {
                    socket.Dispose();
