...
if __name__ == "__main__":
    ...
    # 替换课程操作
    replace_parser = change_lesson_subparsers.add_parser("replace", help="Replace a lesson")
    replace_parser.add_argument("class_index", type=int, help="Class index (0-based)")
    replace_parser.add_argument("subject_id", help="New subject ID (GUID)")
    replace_parser.add_argument("--date", help="Date (YYYY-MM-DD), default is today")

    # 交换课程操作
    swap_parser = change_lesson_subparsers.add_parser("swap", help="Swap two lessons")
    swap_parser.add_argument("class_index1", type=int, help="First class index (0-based)")
    swap_parser.add_argument("class_index2", type=int, help="Second class index (0-based)")
    swap_parser.add_argument("--date", help="Date (YYYY-MM-DD), default is today")

    # 批量替换课程操作
    batch_parser = change_lesson_subparsers.add_parser("batch", help="Batch replace lessons")
    batch_parser.add_argument("changes", help="Changes in JSON format, e.g. '{\"0\": \"subject_id\", \"1\": \"subject_id\"}'")
    batch_parser.add_argument("--date", help="Date (YYYY-MM-DD), default is today")

    # 清除换课操作
    clear_parser = change_lesson_subparsers.add_parser("clear", help="Clear all lesson changes")
    clear_parser.add_argument("--date", help="Date (YYYY-MM-DD), default is today")
    ...
    if args.command == "test" or args.command is None:
        ...
        if args.operation is None:
            print("Error: Please specify an operation (replace, swap, batch, clear)")
            sys.exit(1)
        ...
        if args.date:
            change_lesson_request["date"] = args.date

        if args.operation == "replace":
            change_lesson_request["class_index"] = args.class_index
            change_lesson_request["subject_id"] = args.subject_id
        elif args.operation == "swap":
            change_lesson_request["class_index1"] = args.class_index1
            change_lesson_request["class_index2"] = args.class_index2
        ...
        ok, resp = run_request_with_socket(change_lesson_request)
        if not (ok and resp.get("success")):
            sys.exit(1)
